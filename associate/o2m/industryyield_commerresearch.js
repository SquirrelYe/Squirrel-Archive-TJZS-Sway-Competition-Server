...
const industryyield = require('../../interface/operation/industryyield').industryyield;
...
company.hasMany(industryyield, {
    foreignKey: 'company_id',
    constraints: false
});
industryyield.belongsTo(company, {
    foreignKey: 'company_id',
    constraints: false
});
...
module.exports = {
    ...
    add: function (req, res) {
        co(function* () {
            var industryyield1 = yield industryyield.create({'id':req.query.industryyield_id}); 
            var commerresearch1 = yield commerresearch.findById(req.query.commerresearch_id)  
            yield industryyield1.setCommerresearch(commerresearch1) 
            .then(msg => {
                res.send(msg);
            })
        }).catch(function (e) {
    ...
    del: function (req, res) {
        co(function* () {
            var industryyield1 = yield industryyield.findById(req.query.industryyield_id)   
            yield industryyield1.setCommerresearch(null) 
            .then(msg => {
                res.send(msg);
            })
        }).catch(function (e) {
