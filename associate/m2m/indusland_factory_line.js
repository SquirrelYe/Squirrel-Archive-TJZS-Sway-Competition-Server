...
const indusland_factory = require('../../associate/m2m/indusland_factory').indusland_factory;
...
module.exports = {
    add: function (req, res) {
        co(function* () {
            var indusland_factory1 = yield indusland_factory.create({'id': req.query.indusland_factory_id});
            var line1 = yield line.findById(req.query.line_id);

            yield indusland_factory1.addLine(line1)
            .then(msg => {
                res.send(msg);
            });
        }).catch(function (e) {
    ...
    update: function (req, res) {
        co(function* () {
            var line1 = yield line.findById(req.query.line_id)  
            var indusland_factory1 = yield indusland_factory.findById(req.query.indusland_factory_id)  
            yield indusland_factory1.addLine(line1) 
            .then(msg => {
                res.send(msg);
            })
    ...
    del: function (req, res) {
        co(function* () {
            var indusland_factory1 = yield indusland_factory.findById(req.query.indusland_factory_id) 
            yield indusland_factory1.setLines(null) 
            .then(msg => {
                res.send(msg);
            })
        }).catch(function (e) {
    ...
    find_more_3: function (req, res) {
        indusland_factory.findAll({
            'where':{ 'id':req.query.indusland_factory_id}
        ,
        include: {
            model: line, // 关联查询，关联外键模型
        }}
    ...
    selectNumber:function(req,res){
        indusland_factory_line.findAndCountAll({
                'attributes':['number']
            ,
            'where':{ 
                'indusland_factory_id':req.query.indusland_factory_id,
